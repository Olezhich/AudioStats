...
class PlayListHandler:
    ...
    def process_playlist_paths(self, playlist: list[str]) -> Iterator[AlbumDTO]:
        ...
        for path in playlist:
            ...
            files_total += 1
        logger.info(
            f'Playlist processed in {(time.time() - t_start) * 1000:.3f} ms. Files total: {files_total}, successfully processed: {files_processed}'
        )
            ...
            with open(path, 'r') as f:
                ...
                if cue.rem.date:
                    try:
                        ...
                        logger.warning(
                            f'No correct date in album: {cue.performer} - {cue.title} - {path}'
                        )
                else:
                    logger.warning(
                        f'No any date in album: {cue.performer} - {cue.title} - {path}'
                    )
                ...
                if len(album.tracks) < 1:
                    logger.warning(
                        f'No tracks in album: {cue.performer} - {cue.title} - {path}'
                    )
                    return None
    ...
    def _get_cover_path(self, current_dir: str) -> str | None:
        ...
        for file in os.listdir(current_dir):
            if any(
                file.lower() == i + j
                for i in self._config['CoverNames']
                for j in self._config['CoverExtensions']
            ):
                return os.path.join(current_dir, file)
        return None

    def _process_cue_tracks(
        self, cue: cuetools.AlbumData, current_dir: str
    ) -> Iterator[TrackDTO]:
        offset: float = 0
        duration: float
        for track_cue in sorted(cue.tracks, reverse=True, key=lambda x: int(x.track)):
            offset, duration = (
                self._get_offset_duration(current_dir, track_cue, offset)
    ...
    def _get_offset_duration(
        self, current_dir: str, track_cue: TrackData, next_offset: float | None
    ) -> tuple[float, float]:
        ...
        duration = (
            ...
            else self._get_audiofile_duration(os.path.join(current_dir, track_cue.link))
            - offset
        )
    ...
    def _get_ape_duration(self, path_to_file: str) -> float:
        result = subprocess.run(
            [
                arg.replace('{path_to_file}', path_to_file)
                for arg in self._config['ApeDurationCmd']
            ],
