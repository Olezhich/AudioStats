...
async def test_async_upsert_albums(processed_album_dtos):
    ...
    assert sorted(albums, key=lambda x: x.title) == sorted(
        target_album_dtos, key=lambda x: x.title
    ), 'Insert some albums to db'
    ...
    for i in target_muted_album_dtos:
        i.statuses.append(StatusDTO(status=Status.ADDED, success=Success.SUCCESS))
    target_muted_album_dtos[0].statuses.append(
        StatusDTO(status=Status.MODIFIED, success=Success.SUCCESS)
    )
    target_muted_album_dtos[1].statuses.append(
        StatusDTO(status=Status.MODIFIED, success=Success.SUCCESS)
    )

    assert sorted(albums, key=lambda x: x.title) == sorted(
        target_muted_album_dtos, key=lambda x: x.title
    ), 'Update some albums in db'
